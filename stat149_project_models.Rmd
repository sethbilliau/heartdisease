...
=======
library(rpart)
>>>>>>> 345ae234889a67f56afbbf47c037297c2daad617
```
...
```{r}
gam.final <- gam.init8
anova(m, gam.final, test = "Chisq")
```

=======
# alt modeling approach: classification trees
# note: trees can't handle interaction terms
tree1 = rpart(CHD_Risk ~ ., data = data, method = "class", parms = list(split = "information"))
tree1 # stuck at the root

# try loosening constraints for growth, using 1-SE method to find optimal cp
# from help for rpart.control: "any split that does not decrease the overall lack of fit
# by a factor of cp is not attempted"
tree2 = rpart(CHD_Risk ~ ., data = data, method = "class", parms = list(split = "information"),
              control = rpart.control(minsplit = 1, cp = 0.001))
printcp(tree2)
plotcp(tree2)

tree3 = rpart(CHD_Risk ~ ., data = data, method = "class", parms = list(split = "information"),
              control = rpart.control(minsplit = 1, minbucket = 1, cp = 0.0072464))
tree3
```
>>>>>>> 345ae234889a67f56afbbf47c037297c2daad617
